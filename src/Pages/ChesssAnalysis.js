...
const ChessAnalysis = () => {
  ...
  useEffect(() => {
    if (selectedOption === "Chess" || selectedOption === "LeeChess.org") { // to hide and show button
  ...
  const GetGamesfromServer = async (DateForFetchGames) => {
    await FetchgamesByDate({ selectedOption, UserOptionData, DateForFetchGames, setGamesData, setLoading });
  }
  ...
  useEffect(() => {
    ...
    if (selectedOption === "LeeChess.org") {
      const since = new Date(CurrentDate.getFullYear(), CurrentDate.getMonth(), 1).getTime();
      const until = new Date(CurrentDate.getFullYear(), CurrentDate.getMonth() + 1, 0, 23, 59, 59).getTime();
      // return { since, until };
      const DateForFetchGames = {
        since,
        until
        ...
        <div className="analysis-sidebar bg-black text-white p-4 ml-0 md:ml-4 rounded-lg w-full md:w-1/3 max-w-2xl flex flex-col justify-between">
          <div>
            ...
            <div className="flex justify-center gap-0.5 items-center">
              <input
                type="text"
                placeholder={selectedOption === "Chess" || selectedOption === "LeeChess.org" ?
                  "Enter Username"
                  : (
                    selectedOption === "JSON" ?
                      "Enter JSON"
                ...
                onChange={(e) => { setUserOptionData(e.target.value) }}
              />
        ...
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50">
          <div className="bg-gray-900 text-white p-5 rounded-lg shadow-lg w-2/5">
            ...
            <div className="flex justify-around items-center mb-3">
              ...
              <span>{`${String(CurrentDate.getMonth() + 1).padStart(2, '0')}/${CurrentDate.getFullYear()}`}</span>

              <button
                onClick={handleForwordMonth}>
              ...
              <div className="flex flex-col space-y-2 h-[50vh] overflow-auto">
                {
                  Loading ? (
                    ...
                    GamesData.length > 0 ? GamesData.map((game, index) => (
                      <button key={index} className="p-2 bg-gray-700 rounded text-left flex items-center gap-8 px-6">
                        <span className="text-base font-bold">{game?.time_class || game.speed}</span> <span>{game?.white?.username || game?.players?.white?.user?.name} ({game?.players?.white?.rating || game?.white?.rating})  vs.  {game?.black?.username || game?.players?.black?.user?.name} ({game?.players?.black?.rating || game?.white?.rating})</span>
                      </button>
                    ))
